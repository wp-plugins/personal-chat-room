...
global $wpdb;
$profile_table = "CREATE TABLE `cs_profiles` (
...
$wpdb->query($profile_table); 

$profile_insert = "INSERT INTO `cs_profiles` (`name`, `first_name`, `last_name`, `email`, `password`, `salt`, `status`, `role`, `about`, `date_reg`, `rate`, `rate_count`) VALUES
...
$wpdb->query($profile_insert); 

$message_table = "CREATE TABLE `cs_messages` (
...
$wpdb->query($message_table); 

$message_val_tab = "CREATE TABLE `cs_val_tab` (
  `id` int(11) unsigned NOT NULL auto_increment,
  `value` VARCHAR(255) NOT NULL,
  PRIMARY KEY (`id`) 
)";
$wpdb->query($message_val_tab);
?>