...
/* 	Plugin Name: Personal Chat room
	...
	Description: Personal Chat Room adds a facility to allow your customers to chat with one another. Here They can questions with another users and can get the answers
				from another users of your website.
	Version: 1.1
	Author: lordlinus
...
function chat_room_lord()
{
		if(isset($_POST['save_lord']))
		{
			...
			echo "<font color='red'> Your Data has been updated ... </font><br/>";
		}
		$get_value = get_option('chat_lord_linus_database');
		//print_r($get_value);
		echo "<form action='' method='post'>";
		echo "<h1> Please Set up Your Database here</h1>";
		echo "<table><tr><td>HOST :</td><td> <input type='text' name='lord_linus_host' size='50' value='".$get_value[1]."' required='required'/> </td></tr>";
		echo "<tr><td>Database Name : </td><td><input type='text' name ='lord_linus_dbname' value='".DB_NAME."' size='50' disabled='disabled'></td></tr>";
		echo "<tr><td>Database User name : </td><td><input type='text' name='lord_linus_user' size='50' value='".$get_value[2]."' required='required'></td></tr>";
		echo "<tr><td>Database Password : </td><td><input type='text' name = 'lord_linus_password' size='50' value='".$get_value[3]."' ></td></tr>";
...
$url = plugins_url('/chat/index.php', __FILE__);
...
echo "<div class='middle_left' id='support_btn'><div id='middle_left_text'><form method='post' action='$url' target='_blank'><input type='hidden' name='value' value='$value'><a type='submit' style='text-decoration:none;' href=$url target='_blank' >Live Chat</a></form></div></div>";
}
